...
export const RAW_SAMPLE_SOURCE = [
  ...
  {
    code: "10.0123.0021",
    name: "Chọc hút tế bào bằng kim nhỏ (FNA) tuyến giáp",
  },
  {
    code: "11.0041.0043",
...
export function getSampleSourceList(): SourceItem[] {
  return RAW_SAMPLE_SOURCE.map((item, idx) => {
    // Giả lập phân chia theo chương để minh họa STT 1 cộng dồn và STT 2 theo chương gốc
    const isChapter2 = idx >= 8;
    const chapterName = isChapter2 ? 'II. Chuyên khoa Ngoại & Hồi sức' : 'I. Chương chung';
    const stt2InChapter = isChapter2 ? idx - 8 + 1 : idx + 1;
    ...
    return {
      ...
      name: item.name,
      chapter: chapterName,
      rawRow: [stt2InChapter, item.code, item.name, idx % 2 === 0 ? 'X' : '', 'Khoa Khám bệnh / Ngoại'],
    };
  });
}
