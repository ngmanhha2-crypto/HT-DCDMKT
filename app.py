...
def evaluate_anatomy_compatibility(comp1: dict, comp2: dict) -> dict:
    ...
    - SEVERE_CONFLICT: Xung đột nghiêm trọng (ví dụ Âm hộ vs Bờ mi) -> Chặn 100%
    """
    ...
    if anat1 and anat2:
        ...
        # Xung đột nghiêm trọng bộ phận giữa 2 chuyên khoa khác nhau hoàn toàn
...
class IndexedTargetDict:
    ...
    def match(self, query: str, threshold: float = 0.70, enable_anatomy_filter: bool = True) -> tuple:
        ...
        # 1. Khớp chính xác tuyệt đối O(1)
...
def read_excel_smart_header(file_bytes, target_keywords: list, default_skiprows: int = 1) -> pd.DataFrame:
    ...
    # Đọc thử 20 dòng đầu không header để định vị chính xác
...
def find_column_by_keyword(df: pd.DataFrame, candidates: list, fallback_idx: int = None) -> str:
    ...
    # 2.2. Cột chứa các từ chính của cụm từ khóa
    for cand in norm_candidates:
    ...
    # 3. Fallback theo vị trí cột nếu hợp lệ
...
def process_source_file(file_bytes) -> pd.DataFrame:
    ...
    # Tìm cột Tên theo từ khóa "tên kỹ thuật" hoặc "danh mục kỹ thuật"
    ...
    if not col_code:
        raise ValueError(
            f"File Danh mục gốc không tìm thấy cột mã ('Mã TT 43, 21'). "
            f"Các cột hiện có: {list(df.columns)}"
        )
...
def main():
    ...
    with st.sidebar:
        ...
        enable_anatomy_filter = st.checkbox(
            "🧠 Bộ Lọc Y Tế 2 Tầng (Smart Anatomy Engine)",
            value=True,
            help="Tự động bóc tách Hành động Kỹ thuật can thiệp và Bộ Phận Giải Phẫu tương ứng. Ngăn chặn 100% rủi ro ghép sai chuyên khoa (ví dụ: rách âm hộ ❌ bờ mi)."
        )
        ...
        st.markdown("---")
        st.info("💡 **Gợi ý:** Thuật toán tự động bóc tách bộ phận giải phẫu, làm sạch ký tự nhiễu (*, +, -, ...) và tìm kiếm chuỗi tương đồng cao nhất.")
    ...
    if start_mapping:
        ...
        try:
            ...
            status_text.text("Đang xử lý và tạo chỉ mục tăng tốc Phụ lục 1...")
            dict_pl1 = process_pl1_file(file_pl1)
            index_pl1 = IndexedTargetDict(dict_pl1)
            progress_bar.progress(30)
            
            status_text.text("Đang xử lý và tạo chỉ mục tăng tốc Phụ lục 2...")
            dict_pl2 = process_pl2_file(file_pl2)
            index_pl2 = IndexedTargetDict(dict_pl2)
            progress_bar.progress(45)
            
            total_items = len(df_goc)
            st.info(f"📊 Đã nạp thành công: **{total_items}** mục từ File Gốc | **{len(dict_pl1)}** mục PL1 | **{len(dict_pl2)}** mục PL2 (Đã tạo chỉ mục tăng tốc 2 tầng)")
            ...
            for i, row in df_goc.iterrows():
                ...
                # Khớp với Phụ lục 1 (sử dụng chỉ mục tăng tốc và bộ lọc giải phẫu)
                pl1_name, pl1_code, _ = get_best_match(
                    ten_goc_raw, index_pl1, threshold=threshold, enable_anatomy_filter=enable_anatomy_filter
                )
                
                # Khớp với Phụ lục 2 (sử dụng chỉ mục tăng tốc và bộ lọc giải phẫu)
                pl2_name, pl2_code, _ = get_best_match(
                    ten_goc_raw, index_pl2, threshold=threshold, enable_anatomy_filter=enable_anatomy_filter
                )
